# Saudi Financial Services Voice Agent

> A free Arabic voice agent for Saudi finance teams — onboarding, loan questions and lead qualification in Najdi Arabic, self-hosted.
...
<p align="center">
  <a href="https://voho.ai/industries/financial-services">
    <img src="docs/demo.png" alt="Saudi Financial Services Voice Agent — Client onboarding played end to end in Saudi Arabic, with the tool calls and the outcome visible" width="900">
  </a>
</p>

<p align="center">
  <b><a href="https://voho.ai/industries/financial-services">▶ Play the live demo</a></b> — runs in your browser, no sign-up.
</p>
...
Applications stall for days on questions a person could answer in ninety seconds, if anyone picked up.

This is for finance companies, brokerages and lending teams. Every conversation is in **Najdi Arabic** — the
...
| Scenario | | |
| --- | --- | --- |
| `onboard` | Client onboarding | Identity documents collected and checked before anyone picks up the phone. |
| `qualify` | Lead qualification | Every enquiry answered and sent to the right adviser, none left waiting until Monday. |
| `statement` | Statement and portfolio | Everyday account questions answered straight away, in Saudi Arabic. |

Play any of them right now, before you have connected anything:

```bash
python examples/play.py onboard
```

The tools (`create_application`, `get_portfolio`, `kyc_prefill`, `qualify_lead`, `route_to_advisor`, `send_statement`) replay the results recorded with each scenario, so the
...
| Scenario | | |
| --- | --- | --- |
| `suitability` | Suitability pack review | a hard document read into fields |
| `advisor-note` | Advisor meeting note | speech turned into business-ready text, then an action |
...
```bash
git clone https://github.com/yar-malik/saudi-financial-services-voice-agent.git
cd saudi-financial-services-voice-agent
...
@implement("create_application")
def create_application(product: str, risk: str) -> dict:
    # your system here
    return {"application_id": "APP-3320", "route": "advisor_review"}
```
...
In financial services, that usually means:

| System | What it is for |
| --- | --- |
| **Nafath** | identity, before an application is discussed at all |
| **Tadawul market data** | for the questions that arrive the moment the market moves |
| **SIMAH-style credit checks** | the affordability step, called as a tool rather than guessed at |
| **Your CRM and origination system** | where the application actually lives |
